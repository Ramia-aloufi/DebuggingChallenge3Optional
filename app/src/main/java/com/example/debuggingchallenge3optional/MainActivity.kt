package com.example.debuggingchallenge3optional
...
class MainActivity : AppCompatActivity() {
    ...
    private suspend fun updateRV(result: String){
        withContext(Main){
            ...
            Log.d("jsonArray", "DATA: $jsonArray")

            val main = jsonArray[0]
            Log.d("main", "DATA: $main")

            val word = JSONObject(main.toString()).getString("word")
            Log.d("word", "DATA: $word")

            val inside = jsonArray.getJSONObject(0).getJSONArray("meanings").getJSONObject(0)
            Log.d("inside", "inside: $inside")

            val definitionss = inside.getJSONArray("definitions").getJSONObject(0)

            val definition = JSONObject(definitionss.toString()).getString("definition")
            Log.d("definition", "WORD:$definition")
